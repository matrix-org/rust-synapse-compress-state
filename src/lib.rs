...
/// Struct used to produce SQL code to carry out changes to database.
///
/// It can be iterated over, where each call to `next()` will return the
/// SQL to alter a single state group
///
/// # Fields
///
...
struct DatabaseChanges<'a> {
    old_map: btree_map::Iter<'a, i64, StateGroupEntry>,
    new_map: &'a BTreeMap<i64, StateGroupEntry>,
    room_id: &'a str,
}

impl<'a> DatabaseChanges<'a> {
    /// Create a new DatabaseChanges struct
    fn new(
...
impl Iterator for DatabaseChanges<'_> {
    type Item = String;
...
fn output_sql(
    ...
    if let Some(output) = &mut config.output_file {
        for mut sql_transaction in DatabaseChanges::new(old_map, new_map, &config.room_id) {
